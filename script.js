...
}// Add this line after your existing code to handle the game over state better
function checkAns(idx) {
    if (userSeq[idx] == gameSeq[idx]) {
        if (userSeq.length == gameSeq.length) {
        ...
        highscore = Math.max(level, highscore);
        h2.innerHTML = `Game Over! Your Score was <b>${level}</b> and your highest score was <b>${highscore}</b><br>Press any key to start.`;
